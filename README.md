...
- 입력된 커스텀 구분자와 기본 구분자 이외에 다른 문자가 존재할 경우 -> `[ERROR] 기본 구분자와 일치하지 않는 문자가 존재합니다.` (기본 구분자와 커스텀 구분자 혼용 허용)
- "//"와 "\n" 사이에 **문자열**이 위치할 경우  **ex) //;;?\n** -> `[ERROR] 잘못된 커스텀 구분자 형식입니다.` (문자열은 구분자로 허용하지 않는다.)
- "//"와 "\n" 사이에 아무것도 입력하지 않은 경우 **ex) //\n** -> `[ERROR] 잘못된 커스텀 구분자 형식입니다.` (공백 혹은 빈 문자는 구분자로 허용하지 않는다.)
